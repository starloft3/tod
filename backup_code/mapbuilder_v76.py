...
def drawCaravans():
    for x in range(len(allcaravans)):
        color = convertColor(int(allcaravans[x][2]))
        templist = allcaravans[x]
        templistB = templist[4:]
        route = filter(lambda a: a != None, templistB)
        for z in range(len(route) - 1):

            pygame.draw.line(windowSurface, color, (hexSurfaces[int(route[z])].centerx, hexSurfaces[int(route[z])].centery), (hexSurfaces[int(route[z + 1])].centerx, hexSurfaces[int(route[z + 1])].centery), 6)
...
MENU_SEVEN_TEXTrect.left = 1005 * resX
