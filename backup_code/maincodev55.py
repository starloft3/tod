...
CLIENT_DATA_UNITS=1
CLIENT_DATA_BASES=2
CLIENT_DATA_VISION=3
CLIENT_DATA_BUILDABLES=4
#GLOBALS
...
def loadcurrentfaction():
    currentfactiondata=csv.reader(open('currentfaction.csv'))
    count = 0
...
def expand(actionhex, targetHex):
    for x in range(len(allbases)):
        if allbases[x][BASE_LOCATION] == actionhex and allbases[x][BASE_LUMBER]>0 and (allhexes[targetHex][HEX_TERRAIN]=='F' or allhexes[targetHex][HEX_TERRAIN]=='C' or (allhexes[targetHex][HEX_TERRAIN]=='O' and allhexes[targetHex][HEX_OIL]==1)) and IsItThere(targetHex,allexpandables[x])==1:
...
def resolveExpansions():
    ...
    for x in range(len(toberesolved)):
        ...
        for y in range(0,100):
            print 'test:',randint(0,len(toberesolved[x])-1)
        baseid=-1
...
def buildUnit(actionhex,unit):
    for x in range(len(allbases)):
        if allbases[x][BASE_LOCATION]==actionhex and FOOD_SURPLUS[allbases[x][BASE_FACTION]]>0 and allbases[x][BASE_TIER]>=allstats[unit].mintier and allbases[x][BASE_GOLD]>=allstats[unit].goldcost and allbases[x][BASE_LUMBER]>=allstats[unit].lumbercost and allbases[x][BASE_OIL]>=allstats[unit].oilcost:
...
def economicactions():
    ...
    ACTION_CARAVAN_PATH=2#establish caravan
    ACTION_CARAVAN_ORIGIN_BASE=1
    ACTION_CARAVAN_DESTINATION_BASE=3
    ACTION_CARAVAN_FACTION=5
    ACTION_CARAVAN_TYPE=4
    ...
    for x in range(len(alleconomicactions)):#establish caravan
        if alleconomicactions[x][ACTION_NAME]=='Establish Caravan':
            newcaravan=caravan()
            for y in range(len(alleconomicactions[x])-5):                
                newcaravan.path.append(alleconomicactions[x][y+5])
            for y in range(len(allbases)):
                if allbases[y][BASE_LOCATION]==alleconomicactions[x][ACTION_CARAVAN_ORIGIN_BASE]:
                    newcaravan.originbase=y
            for y in range(len(allbases)):
                if allbases[y][BASE_LOCATION]==alleconomicactions[x][ACTION_CARAVAN_DESTINATION_BASE]:
                    newcaravan.destinationbase=y
            newcaravan.faction=allbases[newcaravan.originbase][BASE_FACTION]
...
def generateVision(currentfaction,turn):
    for x in range(len(allunits)):
    ...
    clientdata[currentfaction][CLIENT_DATA_HEXES]=copy.deepcopy(allhexescurrent)
    clientdata[currentfaction][CLIENT_DATA_UNITS]=copy.deepcopy(allunitscurrent)
    clientdata[currentfaction][CLIENT_DATA_BASES]=copy.deepcopy(allbasescurrent)
    clientdata[currentfaction][CLIENT_DATA_VISION]=copy.deepcopy(visiblehexes)
    clientdata[currentfaction][CLIENT_DATA_BUILDABLES]=copy.deepcopy(allbuildables[currentfaction])
    unitlegend=csv.writer('currentunitslegend.csv','wb')
    baselegend=csv.writer('currentbaseslegend.csv','wb')
    localvisiondata=csv.writer(open('currentvision.csv','wb'))
    localvisiondata.writerow(visiblehexes)
...
def SaveGame():
    ...
    initiative=csv.writer(open('currentinitiative.csv','wb'))
...
def GoAlphaRunStart():
    ...
    while GameStart==0:
        clientlogin=[]
        for x in range(0,17):
            clientlogin.append(0)
        failure=0
        for x in range(len(clientlogin)):
            if clientlogin[x]==0:
                failure=1
        if failure==0:
            GameStart=1
    while GameOver==0:
        generatedata(initiative)
        if playercount(initiative)>0:
            allordersreceived=0
            while allordersreceived==0:
                if checkorders(initiative)==1:
                    loadmergeorders(initiative)
                    resolvestuff()
                    allordersreceived=1
                    if checkendgame()==1:
                        GameOver=1
        ...
        if initiative>15:
            initiative=0
            turn=turn+1
            #diplomacy orders should be resolved here
        if initiative==0 or initiative==6:
            for x in allhexes:
                x[HEX_BATTLE_FOUGHT]=0
def generateKeys():
    for x in range(0,17):
        loginkeys.append(randint(0,10000))
def generatedata(initiative,turn):#search for all factions with the current initiative count and make their data. keep legends and vision local
    for x in range(len(allfactions)):
        if allfactions[x]==initiative:
            #make files if not present
            generateVision(x)
def playercount(initiative):#search for all factions with the current initiative count and count them
    count=0
    for x in range(len(allfactions)):
        if allfactions[x]==initiative:
            count=count+1
    return count
def checkorders(initiative):#check to see if orders are in for all factions on the current initiative
    failure=0
    for x in range(len(allfactions)):
        if allfactions[x]==initiative:
            if clientorders[x][CLIENT_ORDER_KEY]!=loginkeys[x]:
                failure=1
    if failure==1:
        return 0
    else:
        return 1
def loadmergeorders(initiative):#load all the orders and merge them
    for x in range(len(allfactions)):
        if allfactions[x]==initiative:
            visioncontrolchecker(x)
            for y in range(len(clientorders[x][0])):                
                allorders.append(clientorders[x][0][y])
            for y in range(len(clientorders[x][1])):
                allorders.append(clientorders[x][1][y])
            for y in range(len(clientorders[x][2])):
                allorders.append(clientorders[x][2][y])
...
def resolvestuff():
    ...
    economicactions()
    #erase the orders
def visioncontrolchecker(faction):#keep vision locally like legends and check to make sure orders are for controlled units and not to unseen hexes
    orderdeleter=[]
    for x in range(len(clientorders[faction][0])):
        delmark=0
        if allunits[clientorders[faction][0][x][0]][UNIT_FACTION]!=faction:
            delmark=1
        for y in range(1,len(clientorders[faction][0][x])):
            if IsItThere(clientorders[faction][0][x][y],visiblehexes)==0:
                delmark=1
        if delmark==1:
            orderdeleter.append(x)
    orderdeleter.reverse()
    for x in range(len(orderdeleter)):
        del(clientorders[faction][0][orderdeleter[x]])
    specialorderdeleter=[]
    for x in range(len(clientorders[faction][1])):
        if allunits[clientorders[faction][1][x][0]][UNIT_FACTION]!=faction:
            specialorderdeleter.append(x)
    specialorderdeleter.reverse()
    for x in range(len(specialorderdeleter)):
        del(clientorders[faction][1][specialorderdeleter[x]])
    economicorderdeleter=[]
    #how should this work? do you need to see the whole caravan path?
def checkendgame():
    print 'butts'
def TestGameStart():
    loadunits()
