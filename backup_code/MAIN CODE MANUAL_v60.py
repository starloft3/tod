...
def allcombat():#haha this is totally wrong
    for x in range(len(allhexes)):
        if len(buildfactionslist(buildcombatlist(x)))>1 and int(allhexes[x][HEX_BATTLE_FOUGHT])==0 and IsItThere(currentinitiative,buildfactionslist(buildcombatlist(x)))==1:
            combat(x)
