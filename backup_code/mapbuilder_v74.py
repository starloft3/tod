...
def fogMiniMap(hex):
    ...
    #GOTO
    pygame.draw.polygon(windowSurface, TRANSPARENT, ((xleft, ymid), (xmidleft, ytop), (xmidright, ytop), (xright, ymid), (xmidright, ybottom), (xmidleft, ybottom)))
...
def redraw():
    ...
    #fog of war blackhex test
    #testarea = hexSurfaces[40]
    #testarea = testarea.move(-50,-25)
    #windowSurface.blit(fogImage, testarea)
    ...
    pygame.draw.rect(windowSurface, WHITE, ((1003 - xcord/13) * resX, (0 - ycord/19.5) * resY, 105 * resX, 50 * resY), 2)
    ...
    if checkSelected() != -1:
        ...
        if units[checkSelected()].faction == currentfaction:
            ...
            if units[checkSelected()].unitType == "Alliance Transport" or units[checkSelected()].unitType == "Horde Transport":
                ...
                TRANSPORT_ONE_TEXTrect.top = 550 * resY
                ...
                TRANSPORT_TWO_TEXTrect.top = 565 * resY
...
#set up initial map coordinates based on faction #JIMGO
init_coords = setMap()
xcord = init_coords[0]
ycord = init_coords[1]

#set up window
windowSurface = pygame.display.set_mode((WINDOW_WIDTH, WINDOW_LENGTH), pygame.RESIZABLE, 32)
...
#set up black hex image
fogImage = pygame.image.load('images/blackhex.png')
...
class mapUnit:
    def __init__(self, hexnumber, overallfaction, capacity, domain, category, movesleft, movesmax, vision, combat, HP, maxHP, faction, unitType, area, selected, movesMade, destination, orders, alive, transportOne, transportTwo, isLeader):
...
for i in range(len(allhexes)):
    ...
    if int(allhexes[i][HEX_MILL]) != -1:
        expansions.append(mapExpansion(counter, 'Mill', int(allhexes[i][HEX_MILL]), (pygame.Rect((hexToX(counter) - EXPANSION_X_OFFSET, hexToY(counter) - EXPANSION_Y_OFFSET), (EXPANSION_WIDTH, EXPANSION_LENGTH))), hexToOverallFaction(int(allhexes[i][HEX_MILL]))))
...
MENU_FOUR_TEXTrect.top = 675 * resY
...
MENU_FOUR_ALT_TEXTrect.top = 675 * resY
...
MENU_SIX_TEXTrect.top = 650 * resY
...
REST_UNIT_TEXTrect.left = 1005 * resX
...
ASSIST_CONSTRUCTION_TEXTrect.left = 1005 * resX
ASSIST_CONSTRUCTION_TEXTrect.top = 600 * resY

ASSIST_CONSTRUCTION_TEXT_TWO = menuFont.render('Requires 2 gold, 2 lumber.', True, GOLD)
ASSIST_CONSTRUCTION_TEXT_TWOrect = ASSIST_CONSTRUCTION_TEXT_TWO.get_rect()
ASSIST_CONSTRUCTION_TEXT_TWOrect.left = 1005 * resX
...
CONFIRM_UNIT_COST_TEXTrect.left = 1005 * resX
...
CONFIRM_UNIT_YES_TEXTrect.left = 1005 * resX
...
CONFIRM_UNIT_NO_TEXTrect.left = 1005 * resX
...
BOARD_TRANSPORT_TEXTrect.left = 1005 * resX
...
RANGED_FIRE_TEXTrect.left = 1005 * resX
...
UNIT_FIVE_TEXTrect.left = 1005 * resX
    ...
    for event in pygame.event.get():  
            ...
            if event.button == 1:
                ...
                #confirm unit
                    ...
                    if checkSelected() != -1 or checkBaseSelected() != -1:
                        ...
                        if foundUnit == 0:
                            for i in range(len(bases)):
                                # if bases[i].area.collidepoint(pygame.mouse.get_pos()) == 1 and isAllied(bases[i].faction, currentfaction) == 1: #can only select allied bases
                                if bases[i].area.collidepoint(pygame.mouse.get_pos()) == 1: #can select all bases
                                    clearSelected()
                                    bases[i].selected = 1
                redraw()
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_r:
                if checkBaseSelected() != -1 and bases[checkBaseSelected()].faction == currentfaction and ('Rest' not in bases[checkBaseSelected()].first_orders and 'Rest' not in bases[checkBaseSelected()].second_orders and 'Rest' not in bases[checkBaseSelected()].third_orders) and (bases[checkBaseSelected()].first_orders == 'None' or bases[checkBaseSelected()].second_orders == 'None' or bases[checkBaseSelected()].third_orders == 'None'):
                    REST_UNIT_TOGGLE = toggle(REST_UNIT_TOGGLE)
                    redraw()
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_p:
                if checkBaseSelected() != -1 and bases[checkBaseSelected()].faction == currentfaction and (bases[checkBaseSelected()].first_orders != 'Upgrade Base' and bases[checkBaseSelected()].second_orders != 'Upgrade Base' and bases[checkBaseSelected()].third_orders != 'Upgrade Base') and (bases[checkBaseSelected()].first_orders == 'None' or bases[checkBaseSelected()].second_orders == 'None' or bases[checkBaseSelected()].third_orders == 'None'):
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_a:
                if checkBaseSelected() != -1 and bases[checkBaseSelected()].faction == currentfaction and (bases[checkBaseSelected()].first_orders != 'Assist Construction' and bases[checkBaseSelected()].second_orders != 'Assist Construction' and bases[checkBaseSelected()].third_orders != 'Assist Construction') and (bases[checkBaseSelected()].first_orders == 'None' or bases[checkBaseSelected()].second_orders == 'None' or bases[checkBaseSelected()].third_orders == 'None'):
                    ASSIST_CONSTRUCTION_TOGGLE = toggle(ASSIST_CONSTRUCTION_TOGGLE)
                    redraw()
