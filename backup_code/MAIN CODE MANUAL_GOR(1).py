...
def generateExpandables():
    for x in range(len(allbases)):
        ...
        for y in range(len(allexpandables[x])):
            ...
            #add checks for mountain/swamp, no oil slick
        deletethese.sort()
        deletethese.reverse()
        for y in deletethese:
            del allexpandables[x][y]
        print 'legal expandables for ',allbases[x][BASE_NAME],' ',allexpandables[x]
...
def assistConstruction(actionhex,assist):
    for x in range(len(allbases)):
        if allbases[x][BASE_LOCATION]==actionhex and allbases[x][BASE_GOLD]>1 and allbases[x][BASE_LUMBER]>1:####this doesn't seem to check if the assisting base has one oil to do it
            valid=1    
            ...
            if valid==1:
                ...
                if caravan.caravantype=='land':
                    allbases[caravan.originbase][BASE_GOLD]=allbases[caravan.originbase][BASE_GOLD]-2
                    allbases[caravan.originbase][BASE_LUMBER]=allbases[caravan.originbase][BASE_LUMBER]-2
                allhexes[assist.destinationbase][HEX_ASSISTED]=1####no wait maybe this one is supposed to be HEX_ASSISTED actually i am pretty sure it is
def constructBase(actionhex,faction,name):
    for x in range(len(allbases)):
        if (allbases[x][BASE_LOCATION]==actionhex and allbases[x][BASE_TIER]==0) or allhexes[actionhex][HEX_ASSISTED]==1:####is this supposed to be BASE_ASSISTED, not HEX_ASSISTED?
            allbases.append([])
...
def giveBase(actionhex, unit):
    ...
    for x in range(len(allbases)):
        print 'test for ',allbases[x][BASE_NAME],':',allbases[x][BASE_LOCATION] == actionhex,allunits[unit][UNIT_LOCATION],actionhex,allfactions[allunits[unit][UNIT_FACTION]],allfactions[allbases[x][BASE_FACTION]]
...
def expand(actionhex, targetHex):
    for x in range(len(allbases)):
        if allbases[x][BASE_LOCATION] == actionhex and allbases[x][BASE_LUMBER]>0 and (allhexes[targetHex][HEX_TERRAIN]=='F' or allhexes[targetHex][HEX_TERRAIN]=='C' or (allhexes[targetHex][HEX_TERRAIN]=='O' and int(allhexes[targetHex][HEX_OIL])==1)) and IsItThere(targetHex,allexpandables[x])==1:#####jimnir needs this line
...
def establishCaravan(caravan):#add checks to make sure there are no bases in the intermediate steps
    ...
    if valid==1:
        ...
        if caravan.caravantype=='land':
            allbases[caravan.originbase][BASE_GOLD]=allbases[caravan.originbase][BASE_GOLD]-1
            allbases[caravan.originbase][BASE_LUMBER]=allbases[caravan.originbase][BASE_LUMBER]-1
def sendResources(origin,destination,gold,lumber,oil):####add check to make sure parent base has enough resources to send
    foundbases=[origin]
    watersuccess==0
        ...
        for y in range(len(allorders)):#sort moves by hexside crossed
            if len(allorders[y])>1 and (allunits[allorders[y][0]][UNIT_MOVEMENT_REMAINING]>0 or (allunits[allorders[y][0]][UNIT_ROAD_MOVE_REMAINING]==1 and allunits[allorders[y][0]][UNIT_ROAD_MOVE_ONLY]==1)):#are there any moves and does the unit have movement remaining
                ...
                if Adjacent(hexone,hextwo)==1 and canMove(hexone,hextwo,allorders[y][0],combatmove)==1:#check if move legal in terms of hex and hexside terrain being passable.
                    ...
                    allorders[y]=[allorders[y][0]]#delete order down to just unit value to not mess up order IDs for other stuff
            else:
                allorders[y]=[allorders[y][0]]#delete order down to just unit value to not mess up order IDs for other stuff
...
def allcombat():#haha this is totally wrong
    for x in range(len(allhexes)):
        if len(buildfactionslist(buildcombatlist(x)))>1 and int(allhexes[x][HEX_BATTLE_FOUGHT])==0:######add check to make sure current initiative occupies hex
            combat(x)
...
def generateVision(currentfaction):
    visiblehexes=[]
    for x in range(len(allunits)):
        if allfactions[allunits[x][UNIT_FACTION]]==allfactions[currentfaction] and allunits[x][UNIT_ALIVE]==1:######jimnir change
...
def InitSetStuff():
    ...
    for x in range(len(allhexes)):
        allhexes[x][HEX_NEW_COMBAT]=1
def countFood():######jimnir needs the fixed version
    for x in range(len(allbases)):
...
def TestGameSave():
    ...
    loadeconomicactions()
    translateclient()######jimnir needs to move this to after load economic actions
    reset()
    hexsideControlSweep()
    countFood()
    specialorders()
