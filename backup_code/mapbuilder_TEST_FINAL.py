...
def loadcurrentvision():
    ...
    for x in visiondata:
        ...
        allvision.append(int(strippedstring))
...
def fogHexes(hex):
    ...
    for y in range(len(allvision)):
        if allvision[y] in hexlist:
            hexlist.remove(allvision[y])
    return hexlist
...
def drawCaravans():
    for x in range(len(allcaravans)):
        ...
        for z in range(len(route) - 1):
            ...
            pygame.draw.line(windowSurface, color, (hexSurfaces[int(route[z])].centerx, hexSurfaces[int(route[z])].centery), (hexSurfaces[int(route[z + 1])].centerx, hexSurfaces[int(route[z + 1])].centery), 12)
...
def redraw():
    ...
    if QUIT_TOGGLE == 1:
        ...
        windowSurface.blit(CANCEL_CARAVAN_TEXT, CANCEL_CARAVAN_TEXTrect)
        if len(tempcaravanlist) > 1:
        ...
        windowSurface.blit(MENU_ONE_TEXT, MENU_ONE_TEXTrect)#GOTO
...
MENU_FOUR_TEXTrect.top = 680 * resY
...
MENU_FOUR_ALT_TEXTrect.top = 680 * resY
...
MENU_FIVE_TEXTrect.top = 700 * resY
...
MENU_SIX_ON_TEXT = menuFont.render('- ON', True, RED)
...
MENU_SEVEN_ON_TEXT = menuFont.render('- ON', True, RED)
MENU_SEVEN_ON_TEXTrect = MENU_SIX_ON_TEXT.get_rect()
MENU_SEVEN_ON_TEXTrect.left = 1165 * resX
...
MENU_SEVEN_OFF_TEXTrect.left = 1165 * resX
...
CARAVAN_TWO_TEXTrect.top = 615 * resY
...
CARAVAN_THREE_TEXTrect.top = 640 * resY

CANCEL_CARAVAN_TEXT = menuFont.render('CANCEL', True, RED)
CANCEL_CARAVAN_TEXTrect = CANCEL_CARAVAN_TEXT.get_rect()
CANCEL_CARAVAN_TEXTrect.left = 1010 * resX
CANCEL_CARAVAN_TEXTrect.top = 690 * resY
...
UNIT_THREE_TEXT = menuFont.render('R = Ranged fire', True, GOLD)
...
UNIT_FIVE_TEXT = menuFont.render('N = Build base (requires ruins, or clear hex w/leader unit)', True, GOLD)
...
for i in range(len(allvision)):
    # print allvision[i]
    fogOfWarList.remove(allvision[i])
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1:
                ...
                #confirm unit
                    ...
                    if CANCEL_CARAVAN_TEXTrect.collidepoint(pygame.mouse.get_pos()) == 1:
                        CARAVAN_TOGGLE = 0
                    else:                                                        
                        for i in range(len(bases)):
                        ...
                        if CARAVAN_TOGGLE == 1:
                            count = 0
                            for i in range(1117):
                                if hexSurfaces[i].collidepoint(pygame.mouse.get_pos()) == 1 and bases[checkBaseSelected()].hexnumber != i:
