...
def constructBase(actionunit,actionhex):
    ...
    if ruinsfound==0:
        #build new base
        allbases.append([])
        basename=determineBaseName(allunits[actionunit][UNIT_FACTION])
...
def buildbases():
    for x in range(len(basebuildorders)):
        ...
        if similarcount==1:
            ...
            if allhexes[basebuildorders[1]][HEX_ASSISTED]==1:
                if buildfactionlist(buildcombatlist(basebuildorders[1]))==1:
...
def specialorders():
    ...
    for x in range(len(allspecialorders)):
        ...
        if allspecialorders[x][ORDER_NAME]=='Build Base' and buildfactionslist(buildcombatlist(allunits[allspecialorders[ORDER_UNIT]][UNIT_LOCATION]))==1:
            valid=1
            for y in range(len(allbases)):
...
##    ACTION_CARAVAN_TYPE=5
    ...
    for x in range(len(alleconomicactions)):#assist construction
        if alleconomicactions[x][ACTION_NAME]=='Assist Construction' and len(buildfactionslist(buildcombatlist(int(alleconomicactions[x][ACTION_HEX]))))<2:
            ...
            ruinspresent=0
            for y in range(len(allbases)):
                if allbases[y][BASE_LOCATION]==alleconomicactions[x][ACTION_ASSIST_HEX] and allbases[y][BASE_TIER]==0 and buildfactionslist(buildcombatlist(alleconomicactions[x][ACTION_ASSIST_HEX]))<2:
                    for z in range(len(allbases)):
                        if allbases[z][BASE_LOCATION]==alleconomicactions[x][ACTION_HEX]:
                            if allbases[z][BASE_GOLD]>1 and allbases[z][BASE_LUMBER]>1:
                                for a in range(len(allunits)):
                                    if allunits[a][UNIT_LOCATION]==alleconomicactions[x][ACTION_ASSIST_HEX] and isLeader(allunits[a][UNIT_NAME])==1:
                                        valid=1
                                        ruinspresent=1
                if allbases[y][BASE_LOCATION]==alleconomicactions[x][ACTION_ASSIST_HEX] and allbases[y][BASE_TIER]>0:
                    basecount=basecount+1
            if basecount==0 and allhexes[alleconomicactions[x][ACTION_ASSST_HEX]][HEX_TERRAIN]=='C' and buildfactionslist(buildcombatlist(alleconomicactions[x][ACTION_ASSIST_HEX]))<2:
                for z in range(len(allbases)):
                    if allbases[z][BASE_LOCATION]==alleconomicactions[x][ACTION_HEX]:
            ...
            if valid==1:
                ...
                allhexes[alleconomicactions[x][ACTION_ASSIST_HEX]][HEX_ASSISTED]=1
                print "HEX'S CONSTRUCTION HAS BEEN SUCCESSFULLY ASSISTED"
                #deduct resources
                #deduct base action
                #set hex assisted to 1
...
while active==1:#main menu
    choice=ordermenu('Main Menu','Set Start Positions','Generate Turn','Resolve Orders','Quit NYI', 'Generate Faction Turn', 'Generate Initiative Turn', 'Resolve Initiative Turn', 'Test Base Naming', 'Reset Base Names')
    if choice==0:
        TestGameStart()
    if choice==1:
        subactive=1
