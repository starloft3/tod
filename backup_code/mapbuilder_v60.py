...
def drawHexsideControl(hex):
    ...
    pygame.draw.line(windowSurface,
                     ...
                     (hexToX(hex) - 53 + xcord, hexToY(hex) - 93 + ycord),
                     (hexToX(hex) + 60 + xcord, hexToY(hex) - 93 + ycord),
                     5)
    pygame.draw.line(windowSurface,
                     convertColor(allhexes[hex][HEX_NE_CONTROL]),
                     (hexToX(hex) + 60 + xcord, hexToY(hex) - 93 + ycord),
                     (hexToX(hex) + 114 + xcord, hexToY(hex) + 4 + ycord),
                     5)
    pygame.draw.line(windowSurface,
                     convertColor(allhexes[hex][HEX_SE_CONTROL]),
                     (hexToX(hex) + 114 + xcord, hexToY(hex) + 4 + ycord),
                     (hexToX(hex) + 60 + xcord, hexToY(hex) + 101 + ycord),
                     5)
    pygame.draw.line(windowSurface,
                     convertColor(allhexes[hex][HEX_S_CONTROL]),
                     (hexToX(hex) + 60 + xcord, hexToY(hex) + 101 + ycord),
                     (hexToX(hex) - 53 + xcord, hexToY(hex) + 101 + ycord),
                     5)
    pygame.draw.line(windowSurface,
                     convertColor(allhexes[hex][HEX_SW_CONTROL]),
                     (hexToX(hex) - 53 + xcord, hexToY(hex) + 101 + ycord),
                     (hexToX(hex) - 107 + xcord, hexToY(hex) + 4 + ycord),
                     5)
    pygame.draw.line(windowSurface,
            ...
            if event.button == 1:
                ...
                #confirm unit
                    ...
                    if CONFIRM_UNIT_YES_TEXTrect.collidepoint(pygame.mouse.get_pos()) == 1:
                        if bases[checkBaseSelected()].goldleft >= buildunit[1] and bases[checkBaseSelected()].lumberleft >= buildunit[2] and bases[checkBaseSelected()].oilleft >= buildunit[3]:
            ...
            if SEND_RESOURCES_TOGGLE_ONE == 1:
                ...
                if event.key == pygame.K_SPACE:
                    ...
                    if checkBaseSelected() != -1:
                        ...
                        if to_clear_index != -1:
                            if alleconomicactions[i][0] == 'Build Unit':
                                bases[checkBaseSelected()].goldleft += getUnitCost(alleconomicactions[i][2])[1]
                                bases[checkBaseSelected()].lumberleft += getUnitCost(alleconomicactions[i][2])[2]
                                bases[checkBaseSelected()].oilleft += getUnitCost(alleconomicactions[i][2])[3]
