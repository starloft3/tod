...
def loadgame():
    answer = inputbox.ask(windowSurface, "Faction password:")

def loadgametwo():

    print 'Welcome to the Tides of Darkness alpha!'
    print 'Game by Alexander Stevens and Jim Clarke'
    print 'All Warcraft images/concepts property of Blizzard Entertainment'
    print ' '
    print 'Current faction(s):'
    for x in range(len(allfactions)):
        if allfactions[x] == currentinitiative[0] and allturns[x] == 0:
            print factiondictionary[x] + ' - OUTSTANDING'
        elif allfactions[x] == currentinitiative[0] and allturns[x] == 1:
            print factiondictionary[x] + ' - SUBMITTED'
    ...
    print ' '
    print 'Please enter faction password:'
    login_attempt = raw_input()

    match = 0
    for x in range(len(playerdata)):
        if playerdata[x][0] == login_attempt:
    ...
    if match == 1:
        ...
        if allfactions[currentfaction] == currentinitiative[0]:
            ...
            print 'Your faction is not currently accepting orders! Try again later.'
            pygame.quit()
            sys.exit()
...
def loadsavedunits():
    ...
    for x in unitdata:
        ...
        allunits[count][UNIT_COMBAT]=allunits[count][UNIT_COMBAT]+(allunits[count][UNIT_TIER]*5)
        count=count+1
    unitdata_cur.close()
    db.close()
    ...
    if order[0]=='Expand':
        ...
        for x in range(len(allbases)):
            if int(allbases[x][BASE_LOCATION])==int(actionhex):
                print 'allexpandables:',allexpandables[x]
                print 'allvision:',allvision
                print 'targetHex:',targetHex
                print 'actionhex:',actionhex
                print 'hex terrain at target:',allhexes[targetHex][HEX_TERRAIN]
                if (allhexes[targetHex][HEX_TERRAIN]=='F' or allhexes[targetHex][HEX_TERRAIN]=='C' or (allhexes[targetHex][HEX_TERRAIN]=='O' and int(allhexes[targetHex][HEX_OIL])==1)):
                    print 'terrain appropriate!'
                    if IsItThere(targetHex,allexpandables[x])==1:
                        print 'it is in allexpandables'
                        if IntIsItThere(targetHex,allvision)==1:
                            success=1
                            print 'expand order successful!!!!'
                else:
                    success=0
...
##        success=0
##        print 'testing assist construction order:',order
...
##                        if isLeader(unitspresent[y])==1 and int(allfactions[allbases[x][BASE_FACTION]])==int(allfactions[allunits[unitspresent[y]][UNIT_FACTION]]):
##                            success=1
...
def alteraccheck(one,two,unit):#i don't think this will work in the checker for transports because it may not do specialorders first
    ...
    movesize=1
    for x in range(len(allunits)):
        if int(allunits[x][UNIT_FACTION])==FACTION_ALTERAC and IsItThere(allunits[x][UNIT_LOCATION],alterachome)==0 and allunits[x][UNIT_ALIVE]==1:
            foreignunittotal=foreignunittotal+1
            if allunits[x][UNIT_NAME]=='Alliance Transport':
                if allunits[x][UNIT_TRANSPORT_ONE]!=-2:
                    foreignunittotal=foreignunittotal+1
                if allunits[x][UNIT_TRANSPORT_TWO]!=-2:
                    foreignunittotal=foreignunittotal+1
    print 'foreignunittotal:',foreignunittotal
    if allunits[unit][UNIT_TRANSPORT_ONE]!=-2 and allunits[unit][UNIT_TRANSPORT_ONE]!=-1:
        movesize=movesize+1
    if allunits[unit][UNIT_TRANSPORT_TWO]!=-2 and allunits[unit][UNIT_TRANSPORT_TWO]!=-1:
        movesize=movesize+1
    print 'movesize:',movesize
    if (foreignunittotal+movesize)>foreignunitmax and IsItThere(one,alterachome)==1 and IsItThere(two,alterachome)==0:
        print 'THIS SHOULD BE STOPPING THE MOVE'
        return 1
    else:
        return 0
def gilneascheck(one,two,unit):#i don't think this will work in the checker for transports because it may not do specialorders first
    ...
    movesize=1
    for x in range(len(allunits)):
        if int(allunits[x][UNIT_FACTION])==FACTION_GILNEAS and IsItThere(allunits[x][UNIT_LOCATION],gilneashome)==0 and allunits[x][UNIT_ALIVE]==1:
            foreignunittotal=foreignunittotal+1
            if allunits[x][UNIT_NAME]=='Alliance Transport':
                if allunits[x][UNIT_TRANSPORT_ONE]!=-2:
                    foreignunittotal=foreignunittotal+1
    ...
    if allunits[unit][UNIT_TRANSPORT_TWO]!=-2 and allunits[unit][UNIT_TRANSPORT_TWO]!=-1:
        movesize=movesize+1
    if (foreignunittotal+movesize)>foreignunitmax and IsItThere(one,gilneashome)==1 and IsItThere(two,gilneashome)==0:
        return 1
    else:
        return 0
...
def drawBaseFactionHexes(hex, faction):
    ...
    x = miniMapX(hex)
    y = miniMapY(hex)
    xleft = x + int(2 * resXmini)
    if xleft <= 1000:
        xleft = 1001
    ytop = y - int(4 * resYmini)

    windowSurface.lock()
    pygame.draw.rect(windowSurface, BLACK, (xleft, ytop, 6 * resXmini, 6 * resYmini))
    windowSurface.unlock()
...
def drawUnitFactionHexes(hex, faction):
    ...
    windowSurface.lock()
    pygame.draw.rect(windowSurface, BLACK, (xleft, ytop, 4 * resXmini, 4 * resYmini))
    windowSurface.unlock()
...
def drawCaravans():
    for x in range(len(allcaravans)):
        ...
        route = filter(lambda a: a != None, templistB)
...
def redraw():
    ...
    elif checkBaseSelected() != -1 and bases[checkBaseSelected()].faction == currentfaction and bases[checkBaseSelected()].hexnumber not in combathexes:
    ...
    #redraw menu lines
    pygame.draw.line(windowSurface, CHOCOLATE, ((1001 * resX), (0 * resY)), ((1001 * resX), (1000 * resY)), 10)
    pygame.draw.line(windowSurface, CHOCOLATE, ((1001 * resX), (377 * resY)), ((1400 * resX), (377 * resY)), 10)
    pygame.draw.line(windowSurface, CHOCOLATE, ((1001 * resX), (590 * resY)), ((1400 * resX), (590 * resY)), 10)
    ...
    if checkBaseSelected() != -1:
        ...
        BASE_NAME_TEXTrect = BASE_NAME_TEXT.get_rect()
        BASE_NAME_TEXTrect.left = 1200 * resX
        ...
        BASE_FACTION_TEXTrect.left = 1200 * resX
        ...
        BASE_GOLD_TEXTrect.left = 1200 * resX
        ...
        BASE_LUMBER_TEXTrect.left = 1200 * resX
        ...
        BASE_OIL_TEXTrect.left = 1200 * resX
        ...
        BASE_ORDERS_TEXTrect.left = 1200 * resX
        ...
        BASE_ORDERS_TWO_TEXTrect.left = 1200 * resX
        ...
        BASE_ORDERS_THREE_TEXTrect.left = 1200 * resX
        ...
        BASE_ORDERS_FOUR_TEXTrect.left = 1200 * resX
        ...
        BASE_ORDERS_FIVE_TEXTrect.left = 1200 * resX
        ...
        BASE_ORDERS_SIX_TEXTrect.left = 1200 * resX
    ...
    if checkSelected() != -1:
        ...
        FACTION_TEXTrect.left = 1200 * resX
        ...
        COMBAT_TEXTrect.left = 1200 * resX
        ...
        VISION_TEXTrect.left = 1200 * resX
        ...
        ORDERS_TWO_TEXTrect.left = 1200 * resX
        ...
        LIGHT_ARMOR_TEXTrect.left = 1200 * resX
        ...
        HEAVY_ARMOR_TEXTrect.left = 1200 * resX
        ...
        NATURAL_ARMOR_TEXTrect.left = 1200 * resX
        ...
        if units[checkSelected()].faction == currentfaction:
            ...
            if units[checkSelected()].unitType == "Alliance Transport" or units[checkSelected()].unitType == "Horde Transport":
...
#set up window was here

#set up map images and stretch to window
mapImage = pygame.image.load('images/finalterrain5.jpg')#GOTO
...
fogMiniImage = pygame.transform.scale(fogImage, (24, 15))
...
MENU_SEVEN_ON_TEXTrect.left = 1170 * resX
...
MENU_SEVEN_OFF_TEXTrect.left = 1170 * resX
...
EXPAND_TWO_TEXT = menuFont.render('Expansions cost 2 lumber.', True, GOLD)
...
ASSIST_CONSTRUCTION_TEXT_TWOrect.top = 625 * resY
...
ASSIST_CONSTRUCTION_TEXT_THREErect.top = 650 * resY
...
CONFIRM_UNIT_ERROR_TEXTrect.left = 1045 * resX
CONFIRM_UNIT_ERROR_TEXTrect.top = 650 * resY

CONFIRM_UNIT_FOOD_ERROR_TEXT = menuFont.render('- Not enough food!', True, RED)
CONFIRM_UNIT_FOOD_ERROR_TEXTrect = CONFIRM_UNIT_ERROR_TEXT.get_rect()
CONFIRM_UNIT_FOOD_ERROR_TEXTrect.left = 1045 * resX
...
UPGRADE_BASE_ERROR_TEXT = menuFont.render('Not enough resources!', True, RED)
UPGRADE_BASE_ERROR_TEXTrect = UPGRADE_BASE_ERROR_TEXT.get_rect()
UPGRADE_BASE_ERROR_TEXTrect.left = 1010 * resX
UPGRADE_BASE_ERROR_TEXTrect.top = 650 * resY
...
BASE_SEVEN_TEXTrect.left = 1105 * resX
...
BASE_EIGHT_TEXTrect.left = 1105 * resX
...
BASE_NINE_TEXTrect.left = 1105 * resX
...
BASE_TEN_TEXTrect.left = 1105 * resX
...
BASE_ELEVEN_TEXTrect.left = 1105 * resX
...
BASE_TWELVE_TEXTrect.left = 1105 * resX
...
BASE_THIRTEEN_TEXTrect.left = 1205 * resX
BASE_THIRTEEN_TEXTrect.top = 600 * resY
...
UNIT_FOUR_TEXT = menuFont.render('B = Board transport', True, GOLD)
    ...
    for event in pygame.event.get():  
                ...
                #confirm order submission
                    ...
                    for i in range(len(units)):
                        if units[i].area.collidepoint(pygame.mouse.get_pos()) == 1:
                            ...
                            if units[i].capacity > 0 and units[i].faction == currentfaction and units[i].hexnumber == units[checkSelected()].hexnumber:
        ...
        if event.type == pygame.KEYDOWN:
            if SEND_RESOURCES_TOGGLE_ONE == 1:
                ...
                if event.key == pygame.K_RETURN:
                    if bases[checkBaseSelected()].tier == 1:
                        if bases[checkBaseSelected()].goldleft >= 6 and bases[checkBaseSelected()].lumberleft >= 6 and bases[checkBaseSelected()].oilleft >= 2:
                    ...
                    if bases[checkBaseSelected()].tier == 2:
                        if bases[checkBaseSelected()].goldleft >= 8 and bases[checkBaseSelected()].lumberleft >= 8 and bases[checkBaseSelected()].oilleft >= 4:
                            alleconomicactions.append(["Upgrade Base", bases[checkBaseSelected()].hexnumber])
                            if bases[checkBaseSelected()].first_orders == 'None':
                                updateBaseOrderString(checkBaseSelected(), 'Upgrade Base', ordernumber=1)
                            elif bases[checkBaseSelected()].second_orders == 'None':
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_g and GIVE_BASE_TOGGLE == 0:
                if checkBaseSelected() != -1 and bases[checkBaseSelected()].faction == currentfaction and bases[checkBaseSelected()].first_orders == 'None':
                    GIVE_BASE_TOGGLE = 1
                    redraw()
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_e and GIVE_EXPANSION_TOGGLE_ONE == 0:
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_c and COMMERCE_TOGGLE_ONE == 0:
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_n and BUILD_UNIT_TOGGLE == 0:
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_x and EXPAND_TOGGLE == 0:
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_v and CARAVAN_TOGGLE == 0:
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_p and UPGRADE_BASE_TOGGLE == 0:
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_s and SEND_RESOURCES_TOGGLE_ONE == 0:
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_a and ASSIST_CONSTRUCTION_TOGGLE == 0:
