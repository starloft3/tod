...
def combat(battlehex):
    ...
    battlelog.write('Combat '+str(battlehex)+'\n')
    ...
    if int(allhexes[battlehex][HEX_NEW_COMBAT])==1:
        battlelog.write('Combat is new (not continuing)'+'\n')
            ...
            for y in range(len(factionslist)):
                ...
                while attacklastround==1:
                    ...
                    if attacker!=-1 and target!=-1:
                        ...
                        battlelog.write('Attacker: '+factiondictionary[allunits[x][UNIT_FACTION]]+' '+allunits[x][UNIT_NAME]+'\n')
        ...
        battlelog.write('Combat is continuing (not new)'+'\n')
...
def choosetarget(combatlist,attacker):
    ...
    targetnofires=0
    for x in range(len(combatlist)):
        if allunits[combatlist[x]][UNIT_CATEGORY]!=CATEGORY_NO_FIRE and combatlist[x]!=attacker:
            targetnofires=1
    if targetnofires==1:            
        for x in range(len(combatlist)):
...
def calculateDamage(attacker):
    damageDone = 0
    battlelog.write('Attacker combat: '+str(allunits[attacker][UNIT_COMBAT] + allunits[attacker][UNIT_FLANK] + allunits[attacker][UNIT_TERRAIN])+' (stat:'+str(allunits[attacker][UNIT_COMBAT])+',flank:'+str(allunits[attacker][UNIT_FLANK])+',terrain:'+str(allunits[attacker][UNIT_TERRAIN])+')'+'\n')
...
def hitsTaken(damage,defender): # passed damage
    ...
    if (damagesave>0 and allunits[defender][UNIT_LIGHT_CURRENT]>0):
