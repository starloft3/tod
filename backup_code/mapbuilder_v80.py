...
def redraw():
    ...
    if checkSelected() != -1:
        ...
        CATEGORY_TEXTrect.top = 473 * resY
        ...
        LIGHT_ARMOR_TEXTrect.left = 1225 * resX
        LIGHT_ARMOR_TEXTrect.top = 485 * resY
        ...
        HEAVY_ARMOR_TEXTrect.left = 1225 * resX
        HEAVY_ARMOR_TEXTrect.top = 497 * resY
        ...
        NATURAL_ARMOR_TEXTrect.left = 1225 * resX
        NATURAL_ARMOR_TEXTrect.top = 509 * resY
...
class mapUnit:
    def __init__(self, hexnumber, overallfaction, capacity, domain, category, movesleft, movesmax, vision, combat, HP, maxHP, faction, unitType, area, selected, movesMade, destination, orders, alive, transportOne, transportTwo, isLeader, xshift, yshift, lightarmor, heavyarmor, naturalarmor):
...
MENU_SEVEN_TEXTrect.top = 660 * resY
...
MENU_SEVEN_ON_TEXTrect.top = 660 * resY
...
MENU_SEVEN_OFF_TEXTrect.top = 660 * resY
...
ASSIST_CONSTRUCTION_TEXT_THREErect.top = 625 * resY
    ...
    for event in pygame.event.get():  
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_r:
                if checkBaseSelected() != -1 and bases[checkBaseSelected()].faction == currentfaction and ('Rest' not in bases[checkBaseSelected()].first_orders and 'Rest' not in bases[checkBaseSelected()].second_orders and 'Rest' not in bases[checkBaseSelected()].third_orders) and (bases[checkBaseSelected()].first_orders == 'None' or bases[checkBaseSelected()].second_orders == 'None' or bases[checkBaseSelected()].third_orders == 'None'):
                    REST_UNIT_TOGGLE = 1
                    redraw()
