...
def establishCaravan(caravan):#add checks to make sure there are no bases in the intermediate steps
    ...
    if caravan.caravantype=='water':
        for x in range(1,len(caravan.path)-1):
            if Adjacent(caravan.path[x-1],caravan.path[x])==0 or allhexes[caravan.path[x]][HEX_TERRAIN]!='O' or (HexsideTerrain(caravan.path[x-1],caravan.path[x])!='O' and HexsideTerrain(caravan.path[x-1],caravan.path[x])!='K') or EnemyUnitsPresent(caravan.path,caravan.faction)==0:
                valid=0
                print 'water caravan order found illegal because of adjacency or terrain or enemy units being present'
            ...
            if Adjacent(caravan.path[x-1],caravan.path[x])==0:
                ...
                if allhexes[caravan.path[x]][HEX_TERRAIN]=='O' or allhexes[caravan.path[x]][HEX_TERRAIN]=='I':
                    ...
                    if HexsideTerrain(caravan.path[x-1],caravan.path[x])!='C' and HexsideTerrain(caravan.path[x-1],caravan.path[x])!='F' and HasRoad(caravan.path[x-1],caravan.path[x])!=1:
                        ...
                        if EnemyUnitsPresent(caravan.path,caravan.faction)==0:
                            valid=0
                            print 'canceled because enemy units present'
        if Adjacent(caravan.path[len(caravan.path)-1],caravan.path[len(caravan.path)-2])==0 or (HexsideTerrain(caravan.path[len(caravan.path)-1],caravan.path[len(caravan.path)-2])!='C' and HexsideTerrain(caravan.path[len(caravan.path)-1],caravan.path[len(caravan.path)-2])!='F' and HasRoad(caravan.path[len(caravan.path)-1],caravan.path[len(caravan.path)-2])) or allfactions[caravan.faction]!=allfactions[allbases[targetbase][BASE_FACTION]]:
            valid=0
    ...
    if valid==1:
        ...
        if caravan.caravantype=='water':
            allbases[actingbase][BASE_GOLD]=allbases[actingbase][BASE_GOLD]-1
            allbases[actingbase][BASE_LUMBER]=allbases[actingbase][BASE_LUMBER]-1
            allbases[actingbase][BASE_OIL]=allbases[actingbase][BASE_OIL]-1
        if caravan.caravantype=='land':
            allbases[actingbase][BASE_GOLD]=allbases[actingbase][BASE_GOLD]-1
            allbases[actingbase][BASE_LUMBER]=allbases[actingbase][BASE_LUMBER]-1
            ...
            if allunits[allspecialorders[x][ORDER_UNIT]][UNIT_CATEGORY]==CATEGORY_INTERIOR_SIEGE:
                if Adjacent(allunits[allspecialorders[x][ORDER_UNIT]][UNIT_LOCATION],allspecialorders[x][ORDER_TARGET_HEX])==1:
                    if len(buildfactionslist(buildcombatlist(allunits[allspecialorders[x][ORDER_UNIT]][UNIT_LOCATION])))==1:
                        ...
                        #print 'unit added'
        if allspecialorders[x][ORDER_NAME]=='Board Transport' and (allunits[allspecialorders[x][ORDER_TARGET_TRANSPORT]][UNIT_NAME]=='Alliance Transport' or allunits[allspecialorders[x][ORDER_TARGET_TRANSPORT]][UNIT_NAME]=='Horde Transport') and allunits[allspecialorders[x][ORDER_TARGET_TRANSPORT]][UNIT_FACTION]==allunits[allspecialorders[x][ORDER_UNIT]][UNIT_FACTION] and allunits[allspecialorders[x][ORDER_TARGET_TRANSPORT]][UNIT_TRANSPORT_TWO]==-2 and allunits[allspecialorders[x][ORDER_TARGET_TRANSPORT]][UNIT_LOCATION]==allunits[allspecialorders[x][ORDER_UNIT]][UNIT_LOCATION]:##target is a transport of the same faction with extra space in the same hex
...
def moveunits():##road moves into combat?
    ...
    for x in range(len(allunits)):#increase hex durations, set previous locations, set hexside control
        ...
        if allunits[x][UNIT_LOCATION]==allunits[x][UNIT_PREVIOUS_LOCATION]:
            allunits[x][UNIT_HEX_DURATION]=allunits[x][UNIT_HEX_DURATION]+1
        allunits[x][UNIT_PREVIOUS_LOCATION]=allunits[x][UNIT_LOCATION]
        if (allunits[x][UNIT_TRANSPORT_ONE]!=-1 and allunits[x][UNIT_TRANSPORT_ONE]!=-2) and allhexes[allunits[x][UNIT_LOCATION]][HEX_TERRAIN]=='C' and EnemyShipsPresent(x)==0:#disembark transports
            allunits[allunits[x][UNIT_TRANSPORT_ONE]][UNIT_ALIVE]=1
            allunits[allunits[x][UNIT_TRANSPORT_ONE]][UNIT_LOCATION]=allunits[x][UNIT_LOCATION]
            allunits[allunits[x][UNIT_TRANSPORT_ONE]][UNIT_PREVIOUS_LOCATION]=allunits[x][UNIT_LOCATION]
            allunits[allunits[x][UNIT_TRANSPORT_ONE]][UNIT_HEX_DURATION]=0
        ...
        if (allunits[x][UNIT_TRANSPORT_TWO]!=-1 and allunits[x][UNIT_TRANSPORT_TWO]!=-2) and allhexes[allunits[x][UNIT_LOCATION]][HEX_TERRAIN]=='C' and EnemyShipsPresent(x)==0:#disembark transports
            allunits[allunits[x][UNIT_TRANSPORT_TWO]][UNIT_ALIVE]=1
            allunits[allunits[x][UNIT_TRANSPORT_TWO]][UNIT_LOCATION]=allunits[x][UNIT_LOCATION]
            allunits[allunits[x][UNIT_TRANSPORT_TWO]][UNIT_PREVIOUS_LOCATION]=allunits[x][UNIT_LOCATION]
            allunits[allunits[x][UNIT_TRANSPORT_TWO]][UNIT_HEX_DURATION]=0
    ...
    for x in trollplaces:
        ...
        if len(buildfactionslist(buildcombatlist(x)))>0:
            ...
            for y in range(len(whythehelldoineedthis)):
                if 4<whythehelldoineedthis[y]<13:
                    for x in range(len(allunits)):
                        if int(allunits[x][UNIT_FACTION])==trollfaction:
                            allunits[x][UNIT_FACTION]=FACTION_AMANI
                    for x in range(len(allbases)):
                        if int(allbases[x][BASE_FACTION])==trollfaction:
...
def combat(battlehex):
    ...
    print 'combat in hex',battlehex
    print 'combat list for battle:',combatlist
    print 'factions list for battle',factionslist
...
##            allunitscurrent[x][UNIT_TRANSPORT_THREE]=allunitscurrentlegend[allunitscurrent[x][UNIT_TRANSPORT_THREE]]
    ...
    allcaravanscurrent=[]
    for x in range(len(allcaravans)):
        print 'caravan',x,'being saved into client data'
        allcaravanscurrent.append([])
        print allcaravanscurrent[x]
        if IsItThere(allcaravans[x].originbase,visiblehexes)==1:
            print "the caravan's origin is visible"
            allcaravanscurrent[x].append(allcaravans[x].originbase)
            print allcaravanscurrent[x]
        else:
            allcaravanscurrent[x].append(-1)
            print "the caravan's origin is not visible"
            print allcaravanscurrent[x]
        if IsItThere(allcaravans[x].destinationbase,visiblehexes)==1:
            print "the caravan's destination is visible"
            allcaravanscurrent[x].append(allcaravans[x].destinationbase)
            print allcaravanscurrent[x]
        else:
        ...
        allcaravanscurrent[x].append(allcaravans[x].faction)
        print allcaravanscurrent[x]
        print 'adding caravan type'
        allcaravanscurrent[x].append(allcaravans[x].caravantype)
        print allcaravanscurrent[x]
        acceptablepathids=[]
        for y in range(len(allcaravans[x].path)):
            print 'caravan path:',allcaravans[x].path
            if IsItThere(allcaravans[x].path[y],visiblehexes)==1:
                print 'hex',allcaravans[x].path[y],'of path is visible'
                ...
                if IsItThere(y-1,acceptablepathids)==0 and y-1>-1:
                    ...
                    print acceptablepathids
                if IsItThere(y+1,acceptablepathids)==0 and y+1<len(allcaravans[x].path):
        ...
        for y in range(len(acceptablepathids)):
            allcaravanscurrent[x].append(allcaravans[x].path[acceptablepathids[y]])
...
def TestGameSave():
    ...
    economicactions()
    #buildbases()
    if int(currentinitiative[0])==4 or int(currentinitiative[0])==13:
        for x in allhexes:
