...
def loadsavedunits():
    unitdata=csv.reader(open('currentunits.csv'))
    ...
    for x in unitdata:
        ...
        count=count+1
def loadsavedmap():
    mapdata=csv.reader(open('currenthexes.csv'))
    for x in mapdata:
        templist=[]
        for y in range(0,7):
            templist.append(x[y])
...
def loadsavedbases():
    basedata=csv.reader(open('currentbases.csv'))
    for x in basedata:
        templist=[]
        templist.append(x[0])
...
def loadcurrentvision():
    visiondata=csv.reader(open('currentvision.csv'))
    count=0
    for x in visiondata:
        if count==0:
            allvision.append(x)
...
def miniMapScale(x,y):
    ...
    xscale = 11.34
    yscale = 18.59
    xsmoothed = int(((x * xscale) - ((x * xscale) % 100)) * resX)
    ysmoothed = int(((y * yscale) - ((y * yscale) % 100)) * resY)
