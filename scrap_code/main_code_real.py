...
def destroyBase(hex):
    #Update allhexes to show no faction control, ruins as structure.
    allhexes[hex][HEX_FACTION] = 'none'
    allhexes[hex][8] = 'ruins'
    #Iterate through allhexes to destroy all expansions.
    for x in range (len(allhexes)):
        if allhexes[x][11] == hex:
            allhexes[x][11] = 0
        if allhexes[x][12] == hex:
            allhexes[x][12] = 0
        if allhexes[x][13] == hex:
            allhexes[x][13] = 0        
    #Update allbases to give demons faction control, show 0 base tier (ruins), destroy all resources.
...
def specialorders():
    ...
    for x in range(len(allspecialorders)):
        if allspecialorders[x][ORDER_NAME]=='Rangedfire' and allunits[allspecialorders[x][ORDER_UNIT]][UNIT_CATEGORY]==CATEGORY_INTERIOR_SIEGE and Adjacent(allunits[allspecialorders[x][ORDER_UNIT]][UNIT_LOCATION],allspecialorders[x][ORDER_TARGET_HEX])==1:
            allunits.append(allunits[allspecialorders[x][ORDER_UNIT]][0:len(allunits[allspecialorders[x][ORDER_UNIT]])])
...
def canMove(one,two,unit,combatmove):
    ...
    if (allunits[unit][UNIT_TYPE]==TYPE_SEA) and ((allhexes[one][HEX_TERRAIN]=='O' and (terrain=='O' or terrain=='C')) or (allhexes[one][HEX_TERRAIN]=='C' and terrain=='O')) and (sideterrain=='O' or sideterrain=='K'):
        move=1
    if allunits[unit][UNIT_TYPE]==TYPE_AIR and terrain!='X':
        move=1
    ...
    if sideterrain=='X':
        move=0
    if allunits[unit][UNIT_TYPE]==TYPE_GROUND and allunits[unit][UNIT_LOCATION]!=allunits[unit][UNIT_PREVIOUS_LOCATION] and (sideterrain!='C' or sideterrain!='F') and (allunits[unit][UNIT_ROAD_MOVE_ONLY]==0 or HasRoad(one,two)==0):
        move=0
