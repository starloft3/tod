...
def updateBaseOrderString(baseID, ordertype):
    if ordertype == 'Clear':
        bases[baseID].orders = 'None'
    if ordertype == 'Destroy Base':
        bases[baseID].orders = 'Destroy Base'
...
def redraw():
    ...
    #redraw menu info
    if checkSelected() == -1 and checkBaseSelected() == -1:
    ...
    if checkBaseSelected() != -1:
        ...
        BASE_NAME_TEXT = statsFont.render(bases[checkBaseSelected()].name, True, GOLD)
        BASE_NAME_TEXTrect = BASE_NAME_TEXT.get_rect()
        BASE_NAME_TEXTrect.left = 1225 * resX
        BASE_NAME_TEXTrect.top = 400 * resY
        ...
        BASE_GOLD_TEXTrect.top = 455 * resY
        ...
        BASE_LUMBER_TEXTrect.top = 470 * resY
        ...
        BASE_OIL_TEXTrect.top = 485 * resY

        BASE_ORDERS_TEXT = statsFont.render('Base Orders:', True, GOLD)
        BASE_ORDERS_TEXTrect = BASE_ORDERS_TEXT.get_rect()
        BASE_ORDERS_TEXTrect.left = 1225 * resX
        BASE_ORDERS_TEXTrect.top = 550 * resY

        BASE_ORDERS_TWO_TEXT = statsFont.render(bases[checkBaseSelected()].orders, True, GOLD)
        BASE_ORDERS_TWO_TEXTrect = BASE_ORDERS_TWO_TEXT.get_rect()
        BASE_ORDERS_TWO_TEXTrect.left = 1225 * resX
        BASE_ORDERS_TWO_TEXTrect.top = 565 * resY
...
class mapUnit:
    def __init__(self, hexnumber, capacity, domain, category, movesleft, movesmax, vision, combat, HP, maxHP, faction, unitType, area, selected, movesMade, destination, orders):
...
class mapBase:
    def __init__(self, name, selected, gold, lumber, oil, hexnumber, faction, tier, area, overallfaction, orders):
        self.name = name
        self.selected = selected
        self.gold = gold
        self.lumber = lumber
...
BASE_SIX_TEXT = statsFont.render('N = Built Unit', True, GOLD)
...
for i in range(len(allunits)):
    units.append(mapUnit(allunits[i][UNIT_LOCATION], transportCapacity(allunits[i][UNIT_TRANSPORT_ONE], allunits[i][UNIT_TRANSPORT_TWO], allunits[i][UNIT_TRANSPORT_THREE]), allunits[i][UNIT_TYPE], allunits[i][UNIT_CATEGORY], allunits[i][UNIT_MOVEMENT_REMAINING], allunits[i][UNIT_MOVEMENT_MAX], allunits[i][UNIT_VISION], allunits[i][UNIT_COMBAT], allunits[i][UNIT_HIT_POINTS], allunits[i][UNIT_MAX_HIT_POINTS], allunits[i][UNIT_FACTION], allunits[i][UNIT_NAME], (pygame.Rect((hexToX(allunits[i][UNIT_LOCATION]), hexToY(allunits[i][UNIT_LOCATION])), (UNIT_WIDTH, UNIT_LENGTH))), 0, 0, -1, 'None'))

#establish bases list
bases = []
for i in range(len(allbases)):
    bases.append(mapBase(allbases[i][0], 0, allbases[i][BASE_GOLD], allbases[i][BASE_LUMBER], allbases[i][BASE_OIL], allbases[i][1], allbases[i][2], allbases[i][3], (pygame.Rect((hexToX(allbases[i][1]) - 60, hexToY(allbases[i][1]) - 60), (BASE_WIDTH, BASE_LENGTH))), hordeOrAlliance(allbases[i][2]), 'None'))

#establish expansions list
expansions = []
counter = 0
for i in range(len(allhexes)):
    if int(allhexes[i][HEX_FARM]) != 0:
...
#establish fog of war list (currently Lordaeron)
visibleHexList = establishVisibility(FACTION_LORDAERON)
fogOfWarList = []
for i in range(1117):
    fogOfWarList.append(i)
for i in range(len(visibleHexList)):
    fogOfWarList.remove(visibleHexList[i])
...
while True:
    ...
    for event in pygame.event.get():  
        ...
        #use spacebar to clear all orders for a selected unit
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_SPACE:
                ...
                if checkBaseSelected() != -1:
                    ...
                    if to_clear_index != -1:
                        ...
                        updateBaseOrderString(checkBaseSelected(), 'Clear')
                        redraw()
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_r:
                if checkSelected() != -1 and units[checkSelected()].category == CATEGORY_INTERIOR_SIEGE and units[checkSelected()].orders == 'None':
                    RANGED_TOGGLE = toggle(RANGED_TOGGLE)
                    
        #hit B to board transport
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_b:
                if checkSelected() != -1 and units[checkSelected()].domain == TYPE_GROUND and units[checkSelected()].orders == 'None':
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_d:
                if checkBaseSelected() != -1:
                    ...
                    updateBaseOrderString(checkBaseSelected(), 'Destroy Base')
                    redraw()
