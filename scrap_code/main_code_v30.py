...
def loadeconomicactions():
    orderdata=csv.reader(open('economicactions.csv'))
    for x in orderdata:
        templist=[]
        for y in range(len(x)):
            if y==0:
...
def loadsavedbuildables():
    orderdata=csv.reader(open('savedbuildables.csv'))
    for x in orderdata:
        templist=[]
        for y in range(len(x)):
            templist.append(int(x[y]))
        allbuildables.append(templist)
def loadcurrentbuildables():#give this to client
    orderdata=csv.reader(open('currentbuildables.csv'))
    count=0
    for x in orderdata:
        if count==0:
...
def loadcurrentunitslegend():
    data=csv.reader(open('currentunitslegend.csv'))
    unitslegend=data
def loadcurrentbaseslegend():
    data=csv.reader(open('currentbaseslegend.csv'))
    baseslegend=data
    ...
    for x in range(len(allbases)):
        ...
        for y in range(len(allexpandables[x])):
            ...
            if int(allhexes[allexpandables[x][y]][HEX_FARM])!=-1 or int(allhexes[allexpandables[x][y]][HEX_MILL])!=-1 or int(allhexes[allexpandables[x][y]][HEX_RIG])!=-1:
                deletethese.append(y)
                allpotentialexpansionsreceived.append(allexpandables[x][y])
        deletethese.sort()
        deletethese.reverse()
...
def assistConstruction(assist):
    for x in range(len(allbases)):
        if allbases[x][BASE_LOCATION]==hex and allbases[x][BASE_GOLD]>1 and allbases[x][BASE_LUMBER]>1:
            valid=1    
            ...
            if valid==1:
                ...
                if caravan.caravantype=='land':
                    allbases[caravan.originbase][BASE_GOLD]=allbases[caravan.originbase][BASE_GOLD]-2
                    allbases[caravan.originbase][BASE_LUMBER]=allbases[caravan.originbase][BASE_LUMBER]-2
                allhexes[assist.destinationbase][BASE_ASSISTED]=1
def constructBase(hex,faction,name):
    for x in range(len(allbases)):
        if (allbases[x][BASE_LOCATION]==hex and allbases[x][BASE_TIER]==0) or allhexes[hex][HEX_ASSISTED]==1:
            allbases.append([])
            allbases[len(allbases)-1].append(name)
            allbases[len(allbases)-1].append(hex)
...
def destroyBase(hex):
    ...
    for x in range (len(allhexes)):
        print hex
        if int(allhexes[x][HEX_FARM]) == hex:
            allhexes[x][HEX_FARM] = -1
        if int(allhexes[x][HEX_MILL]) == hex:
            allhexes[x][HEX_MILL] = -1
        if int(allhexes[x][HEX_RIG]) == hex:
            allhexes[x][HEX_RIG] = -1        
                

    #Update allbases to show 0 base tier (ruins), destroy all resources.
    for x in range(len(allbases)):
        if allbases[x][BASE_LOCATION] == hex:
            allbases[x][BASE_TIER] = 0
            allbases[x][BASE_GOLD] = 0
            allbases[x][BASE_LUMBER] = 0
            allbases[x][BASE_OIL] = 0

def giveExpansion(hex,recipientbase,base):#set the expansions to a crazy negative then assign them to the new bases later after harvest happens
    valid=1
    if allfactions[allbases[recipientbase][BASE_FACTION]]!=allfactions[allbases[base][BASE_FACTION]]:
        valid=0
    if allhexes[hex][BASE_FARM]!=allbases[base][BASE_LOCATION] and allhexes[hex][BASE_RIG]!=allbases[base][BASE_LOCATION] and allhexes[hex][BASE_MILL]!=allbases[base][BASE_LOCATION]:
        valid=0
    if IsItThere(hex,allpotentialexpansionsreceived[recipientbase])==0:
        valid=0
    if valid==1:
        if allhexes[hex][HEX_TERRAIN]=='C':
            allhexes[hex][HEX_FARM]=(allbases[recipientbase][BASE_LOCATION]*-1)-2
        if allhexes[hex][HEX_TERRAIN]=='F':
            allhexes[hex][HEX_MILL]=(allbases[recipientbase][BASE_LOCATION]*-1)-2
        if allhexes[hex][HEX_TERRAIN]=='O':
            allhexes[hex][HEX_RIG]=(allbases[recipientbase][BASE_LOCATION]*-1)-2
        allbases[base][BASE_ACTIONS]=allbases[base][BASE_ACTIONS]-1
def giveBase(hex, unit):

    #Update faction control in allbases.
    for x in range(len(allbases)):
        if allbases[x][BASE_LOCATION] == hex and allunits[unit][UNIT_LOCATION]==hex and allfactions[allunits[unit][UNIT_FACTION]]==allfactions[allbases[x][BASE_FACTION]]:
            allbases[x][BASE_FACTION] = allunits[unit][UNIT_FACTION]
            allbases[x][BASE_ACTIONS]=0

def commerce(hex, resourceIn, resourceOut):

    #Update resource totals in allbases.
    for x in range(len(allbases)):
        if allbases[x][BASE_LOCATION] == hex and allbases[x][resourceOut]>1:
...
def harvest(hex):
    ...
    for x in range(len(allbases)):
        if allbases[x][BASE_LOCATION] == hex:
            allbases[x][BASE_GOLD]=allbases[x][BASE_GOLD]+int(allhexes[hex][HEX_GOLD])
            for y in range(len(allhexes)):
                if int(allhexes[y][HEX_MILL])==hex:
                    print 'mill found'
                    allbases[x][BASE_LUMBER]=allbases[x][BASE_LUMBER]+1
                    allbases[x][BASE_GOLD]=allbases[x][BASE_GOLD]+int(allhexes[y][HEX_GOLD])
                if int(allhexes[y][HEX_FARM])==hex:
                    allbases[x][BASE_GOLD]=allbases[x][BASE_GOLD]+int(allhexes[y][HEX_GOLD])
                if int(allhexes[y][HEX_RIG])==hex:
                    allbases[x][BASE_OIL]=allbases[x][BASE_OIL]+1
            print 'yes'
            print 'aerie peak resources:',allbases[x][BASE_GOLD],allbases[x][BASE_LUMBER],allbases[x][BASE_OIL]
            ResourceLimit()
            print 'aerie peak resources:',allbases[x][BASE_GOLD],allbases[x][BASE_LUMBER],allbases[x][BASE_OIL]
            allbases[x][BASE_ACTIONS]=allbases[x][BASE_ACTIONS]-1

def expand(hex, targetHex):
    for x in range(len(allbases)):
        if allbases[x][BASE_LOCATION] == hex and allbases[x][BASE_LUMBER]>0 and (allhexes[targetHex][HEX_TERRAIN]=='F' or allhexes[targetHex][HEX_TERRAIN]=='C' or (allhexes[targetHex][HEX_TERRAIN]=='O' and allhexes[targetHex][HEX_OIL]==1)) and IsItThere(targetHex,allexpandables[x])==1:
            successfulexpansionorder=[hex,targetHex]
            successfulexpansionorders.append(successfulexpansionorder)
            allbases[x][BASE_ACTIONS]=allbases[x][BASE_ACTIONS]-1
def resolveExpansions():
    toberesolved=[]
    for x in range(len(successfulexpansionorders)):
        copy=0
        copytarget=0
    ...
    for x in range(len(toberesolved)):
        pick=randint(0,len(toberesolved[x]))
        allbases[toberesolved[x][pick][0]][BASE_LUMBER] = allbases[toberesolved[x][pick][0]][BASE_LUMBER] - 1
        #Update allhexes with new expansion information.
        if allhexes[toberesolved[x][pick][1]][HEX_TERRAIN]=='F':
            allhexes[toberesolved[x][pick][1]][HEX_MILL]=hex
        if allhexes[toberesolved[x][pick][1]][HEX_TERRAIN]=='C':
            allhexes[toberesolved[x][pick][1]][HEX_FARM]=hex
        if allhexes[toberesolved[x][pick][1]][HEX_TERRAIN]=='O' and allhexes[targetHex][HEX_OIL]==1:
            allhexes[toberesolved[x][pick][1]][HEX_MILL]=hex
        allbases[toberesolved[x][pick][0]][BASE_ACTIONS]=allbases[toberesolved[x][pick][0]][BASE_ACTIONS]-1
...
def upgradeBase(hex):
    ...
    for x in range(len(allbases)):
        if allbases[x][BASE_LOCATION] == hex and ((allbases[x][BASE_TIER]==1 and allbases[x][BASE_GOLD]>5 and allbases[x][BASE_LUMBER]>5) or (allbases[x][BASE_TIER]==2 and allbases[x][BASE_GOLD]>7 and allbases[x][BASE_LUMBER]>7 and allbases[x][BASE_OIL]>3)):

            #Tier 1 to Tier 2
            if allbases[x][BASE_TIER] == 1:
            ...
            allbases[x][BASE_ACTIONS]=allbases[x][BASE_ACTIONS]-1
def restUnit(hex,unit):
    for x in range(len(allbases)):
        success=0
        goldcost=0
        lumbercost=0
        oilcost=0
        if allbases[x][BASE_LOCATION]==hex and allunits[unit][UNIT_LOCATION]==hex and allunits[unit][UNIT_HEX_DURATION]>1:#hex duration may need to be >2
...
def buildUnit(hex,unit):
    for x in range(len(allbases)):
        if allbases[x][BASE_LOCATION]==hex and FOOD_SURPLUS[allbases[x][BASE_FACTION]]>0 and allbases[x][BASE_TIER]>=unitstats[unit].mintier and allbases[x][BASE_GOLD]>=allstats[unit].goldcost and allbases[x][BASE_LUMBER]>=allstats[unit].lumbercost and allbases[x][BASE_OIL]>=allstats[unit].oilcost:
            allbases[x][BASE_GOLD]=allbases[x][BASE_GOLD]-allstats[unit].goldcost
            allbases[x][BASE_LUMBER]=allbases[x][BASE_LUMBER]-allstats[unit].lumbercost
            allbases[x][BASE_OIL]=allbases[x][BASE_OIL]-allstats[unit].oilcost
            FOOD_SURPLUS[allbases[x][BASE_FACTION]]=FOOD_SURPLUS[allbases[x][BASE_FACTION]]-1
            allunits.append([])
            allunits[len(allunits)-1].append(unit)
            allunits[len(allunits)-1].append(unitstats[unit].maxhp)
            allunits[len(allunits)-1].append(unitstats[unit].combat)
            allunits[len(allunits)-1].append(unitstats[unit].category)
            allunits[len(allunits)-1].append(unitstats[unit].type)
            allunits[len(allunits)-1].append(unitstats[unit].light)
            allunits[len(allunits)-1].append(unitstats[unit].heavy)
            allunits[len(allunits)-1].append(unitstats[unit].natural)
            allunits[len(allunits)-1].append(unitstats[unit].movement)
            allunits[len(allunits)-1].append(unitstats[unit].vision)
...
def economicactions():
    ...
    for x in range(len(alleconomicactions)):#destroy base
        if alleconomicactions[x][ACTION_NAME]=='Destroy Base':
            destroyBase(alleconomicactions[x][ACTION_HEX])
    for x in range(len(alleconomicactions)):#give base
        if alleconomicactions[x][ACTION_NAME]=='Give Base':
            giveBase(alleconomicactions[x][ACTION_HEX],alleconomicactions[x][ACTION_UNIT])
    for x in range(len(alleconomicactions)):#give expansion
        if alleconomicactions[x][ACTION_NAME]=='Give Expansion':
            giveExpansion(alleconomicactions[x][ACTION_HEX],alleconomicactions[x][ACTION_RECIPIENT_BASE],alleconomicactions[x][ACTION_OWNER_BASE])
    for x in range(len(alleconomicactions)):#harvest
        if alleconomicactions[x][ACTION_NAME]=='Harvest':
            harvest(alleconomicactions[x][ACTION_HEX])
    for x in range(len(alleconomicactions)):#commerce
        if alleconomicactions[x][ACTION_NAME]=='Commerce':
            commerce(alleconomicactions[x][ACTION_HEX],alleconomicactions[x][ACTION_RESOURCE_IN],alleconomicactions[x][ACTION_RESOURCE_OUT])
    #upgrade unit NYI
    for x in range(len(alleconomicactions)):#build unit
        if alleconomicactions[x][ACTION_NAME]=='Build Unit':
            buildUnit(alleconomicactions[x][ACTION_HEX],alleconomicactions[x][ACTION_UNIT_TYPE])
    for x in range(len(alleconomicactions)):#expand
        if alleconomicactions[x][ACTION_NAME]=='Expand':
            expand(alleconomicactions[x][ACTION_HEX],alleconomicactions[x][ACTION_EXPAND_HEX])
    resolveExpansions()
    ...
    for x in range(len(alleconomicactions)):#upgrade base
        if alleconomicactions[x][ACTION_NAME]=='Upgrade Base':
            upgradeBase(alleconomicactions[x][ACTION_HEX])
    ...
    for x in range(1,5):#work one destination at a time
        ...
        for y in range(len(hexsidelimitchecks)):#compare hexsidelimitchecks to actual hexside limits and pick losers. delete unsuccessful units from HLC, delete all further orders for unsuccessful units
            while (len(hexsidelimitchecks[y])-3)>hexsidelimitchecks[y][2]:##something wrong is happening here sometimes i think
                ...
                pick=randint(0,len(hexsidelimitchecks[y]))-1
...
def generateVision(currentfaction):
    visiblehexes=[]
    for x in range(len(allunits)):
        if allfactions[allunits[x][UNIT_FACTION]]==allfactions[currentfaction]:
            print 'a unit is granting vision'
            currentvisible=[allunits[x][UNIT_LOCATION]]
            for count in range(0,allunits[x][UNIT_VISION]):
                newadditions=[]
                for y in range(len(currentvisible)):
                    if Adjacent(currentvisible[y],currentvisible[y]+1)==1 and HexsideTerrain(currentvisible[y],currentvisible[y]+1)!='X':
                        newadditions.append(currentvisible[y]+1)
                    if Adjacent(currentvisible[y],currentvisible[y]-1)==1 and HexsideTerrain(currentvisible[y],currentvisible[y]-1)!='X':
                        newadditions.append(currentvisible[y]-1)
                    if Adjacent(currentvisible[y],currentvisible[y]+38)==1 and HexsideTerrain(currentvisible[y],currentvisible[y]+38)!='X':
                        newadditions.append(currentvisible[y]+38)
                    if Adjacent(currentvisible[y],currentvisible[y]-38)==1 and HexsideTerrain(currentvisible[y],currentvisible[y]-38)!='X':
                        newadditions.append(currentvisible[y]-38)
                    if Adjacent(currentvisible[y],currentvisible[y]+39)==1 and HexsideTerrain(currentvisible[y],currentvisible[y]+39)!='X':
    ...
    hexdata=csv.writer(open('currenthexes.csv','wb'))
    unitdata=csv.writer(open('currentunits.csv','wb'))
    basedata=csv.writer(open('currentbases.csv','wb'))
    unitlegend=csv.writer(open('currentunitslegend.csv','wb'))
    baselegend=csv.writer(open('currentbaseslegend.csv','wb'))
    playerdata=csv.writer(open('currentfaction.csv','wb'))#this should later also include turn number
    visiondata=csv.writer(open('currentvision.csv','wb'))
    buildabledata=csv.writer(open('currentbuildables.csv','wb'))
    playerdata.writerow([currentfaction])
    visiondata.writerow(visiblehexes)
    buildabledata.writerow(allbuildables[currentfaction])
    for x in allunitscurrentlegend:
        unitlegend.writerow([x])
    for x in allbasescurrentlegend:
        baselegend.writerow([x])
...
def SaveGame():
    ...
    buildabledata=csv.writer(open('savebuildables.csv','wb'))
    for x in allbuildables:
        buildabledata.writerow(x)
    for x in allhexes:
        hexdata.writerow(x)    
    for x in allunits:
        saverow=[]
...
##        allorders[x][0]=unitslegend[allorders[x][0]]
...
##            alleconomicsactions[x][?]=unitslegend[alleconomicsactions[x][?]]
##    for x in range(len(allspecialorders)):
##        allspecialorders[x][1]=unitslegend[allspecialorders[x][1]]
##        if allspecialorders[x][0]=='Board Transport':
##            allspecialorders[x][ORDER_TARGET_TRANSPORT]=unitslegend[allspecialorders[x][ORDER_TARGET_TRANSPORT]]
def TestGameStart():
    loadunits()
    loadmap()
    loadroads()
...
while active==1:#main menu
    choice=ordermenu('Main Menu','Set Start Positions','Generate Turn','Resolve Orders','Quit NYI')
    if choice==0:
        TestGameStart()
    if choice==1:
        subactive=1
